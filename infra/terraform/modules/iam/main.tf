# IAM Module - OIDC for GitHub Actions + ECS Task Roles
...
# IAM Role for GitHub Actions (Infrastructure - Terraform)
resource "aws_iam_role" "github_actions_infra" {
  name = "${local.name_prefix}-gha-infra-role"
...
# Policy for Infrastructure Role (Terraform permissions)
...
# IAM Role for GitHub Actions (Deployment - ECR/ECS)
...
# Policy for Deployment Role (ECR + ECS)
...
# IAM Role for ECS Task Execution (managed by AWS)
...
# Policy for ECS Tasks (S3, SES, Secrets Manager)
